...
int main(int argc, char** argv) {
	...
	printf("testing polyphonic mode {");
	{
		...
		printf("success\n");
		printf("\ttesting running status with MIDI CLOCK ");
		init_variables();
		mode[playmode].init();
		assert(midibuffer_put(&midi_buffer, NOTE_ON(midi_channel))==true);
		assert(midibuffer_put(&midi_buffer, a.byte[1])==true);
		assert(midibuffer_put(&midi_buffer, a.byte[2])==true);
		assert(midibuffer_put(&midi_buffer, CLOCK_START)==true);
		assert(midibuffer_put(&midi_buffer, b.byte[1])==true);
		...
		assert(midibuffer_put(&midi_buffer, CLOCK_SIGNAL)==true);
		assert(midibuffer_put(&midi_buffer, a.byte[1])==true);
		assert(midibuffer_put(&midi_buffer, 0)==true);
		assert(midibuffer_put(&midi_buffer, CLOCK_SIGNAL)==true);
		assert(midibuffer_put(&midi_buffer, c.byte[1])==true);
		assert(midibuffer_put(&midi_buffer, CLOCK_SIGNAL)==true);
		assert(midibuffer_put(&midi_buffer, 0)==true);
		...
		printf("\trunning status, note and song position\n");
		init_variables();
		mode[playmode].init();
		assert(midibuffer_put(&midi_buffer, CLOCK_START) == true);
		assert(midibuffer_tick(&midi_buffer) == false);
		assert(midibuffer_put(&midi_buffer, 0xF2) == true);
		assert(midibuffer_tick(&midi_buffer) == false);
		assert(midibuffer_put(&midi_buffer, 0x77) == true);
		assert(midibuffer_tick(&midi_buffer) == false);
		assert(midibuffer_put(&midi_buffer, 0x55) == true);
		assert(midibuffer_tick(&midi_buffer) == false);
		assert(midibuffer_put(&midi_buffer, NOTE_ON(midi_channel)) == true);
		assert(midibuffer_tick(&midi_buffer) == false);
		assert(midibuffer_put(&midi_buffer, CLOCK_SIGNAL) == true);
		...
		assert(midibuffer_put(&midi_buffer, CLOCK_SIGNAL) == true);
		assert(midibuffer_tick(&midi_buffer) == false);
		assert(midibuffer_put(&midi_buffer, 0xF1) == true);
		assert(midibuffer_tick(&midi_buffer) == false);
		assert(midibuffer_put(&midi_buffer, 0x01) == true);
		assert(midibuffer_tick(&midi_buffer) == false);
		assert(midibuffer_put(&midi_buffer, CLOCK_SIGNAL) == true);
		assert(midibuffer_tick(&midi_buffer) == false);
		assert(midibuffer_put(&midi_buffer, CLOCK_SIGNAL) == true);
